...
then
	printf ':%saaa1 aaa? bbb? ccc? ./?dd* fff1? :%s*4 ' "$EXCLUDE_PATTERN" "$EXCLUDE_PATTERN" | tr ' ' '\n' >.git/pathspec_for_test
else
	printf ':%saaa1 aaa? bbb? ccc? ./?dd* fff1? :%s*4 ' "$EXCLUDE_PATTERN" "$EXCLUDE_PATTERN" | tr ' ' '\0' >.git/pathspec_for_test
fi
{
