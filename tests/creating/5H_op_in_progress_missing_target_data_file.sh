...
assert_exit_code 2 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $QUIET_FLAGS $COLOR_FLAGS $SUMMARY_FLAGS
...
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $QUIET_FLAGS "$CONTINUE_FLAG" $COLOR_FLAGS $SUMMARY_FLAGS
