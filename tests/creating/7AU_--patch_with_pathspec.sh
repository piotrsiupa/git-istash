...
new_stash_sha_CO="$(
	{
		...
		then
			...
			assert_exit_code 0 git istash "$CREATE_OPERATION" 'aaa0' $UNTRACKED_FLAGS $QUIET_FLAGS $ALL_FLAGS $KEEP_INDEX_FLAGS $COLOR_FLAGS $STAGED_FLAGS $UNSTAGED_FLAGS 'bbb?' --patch -m 'a very controlled stash' $EOI '*5' './?dd*' 'fff1?'
		else
			#shellcheck disable=SC2086
			assert_exit_code 0 git istash "$CREATE_OPERATION" $UNTRACKED_FLAGS $QUIET_FLAGS $ALL_FLAGS $KEEP_INDEX_FLAGS $COLOR_FLAGS $STAGED_FLAGS $UNSTAGED_FLAGS -m 'a very controlled stash' --patch $PATHSPEC_NULL_FLAGS "$PATHSPEC_FROM_FILE_FLAG" .git/pathspec_for_test
		fi
