...
assert_exit_code 2 git $ADVICE_FLAGS istash "$OTHER_APPLY_OPERATION" $SUMMARY_FLAGS $QUIET_FLAGS $COLOR_FLAGS
...
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash "$OTHER_APPLY_OPERATION" "$CONTINUE_FLAG" $SUMMARY_FLAGS $QUIET_FLAGS $COLOR_FLAGS
