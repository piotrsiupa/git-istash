...
assert_exit_code 2 git istash $QUIET_FLAGS "$OTHER_APPLY_OPERATION" $COLOR_FLAGS $SUMMARY_FLAGS
...
#shellcheck disable=SC2086
assert_exit_code 1 git istash $QUIET_FLAGS "$CREATE_OPERATION" $COLOR_FLAGS
...
#shellcheck disable=SC2086
assert_exit_code 0 git istash $QUIET_FLAGS "$OTHER_APPLY_OPERATION" $COLOR_FLAGS "$CONTINUE_FLAG" $SUMMARY_FLAGS
