...
then
	printf 'aaa0 bbb? *ore?0 ./?dd* ' | tr ' ' '\n' >.git/pathspec_for_test
else
	printf 'aaa0 bbb? *ore?0 ./?dd* ' | tr ' ' '\0' >.git/pathspec_for_test
fi
if IS_PATHSPEC_IN_ARGS
then
	#shellcheck disable=SC2086
	assert_exit_code 0 git istash push 'aaa0' $UNTRACKED_FLAGS $ALL_FLAGS $KEEP_INDEX_FLAGS $UNSTAGED_FLAGS $STAGED_FLAGS 'bbb?' $EOI '*ore?0' './?dd*'
