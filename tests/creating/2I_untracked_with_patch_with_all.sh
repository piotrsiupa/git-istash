...
new_stash_sha_CO="$(assert_exit_code 0 git istash "$CREATE_OPERATION" $UNTRACKED_FLAGS $ALL_FLAGS --patch $KEEP_INDEX_FLAGS $UNSTAGED_FLAGS $STAGED_FLAGS -message <.git/answers_for_patch)"
assert_files_HTCO '
?? aaa		aaa
?? bbb		bbb
!! ignored0	ignored0
