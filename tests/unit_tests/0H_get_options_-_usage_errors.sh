...
#shellcheck disable=SC2086
assert_exit_code 2 "$GET_OPTIONS_COMMAND" $MODE_FLAGS 'ab:cad' ''
...
#shellcheck disable=SC2086
assert_exit_code 2 "$GET_OPTIONS_COMMAND" $MODE_FLAGS 'ab:cb:d' ''
...
#shellcheck disable=SC2086
assert_exit_code 2 "$GET_OPTIONS_COMMAND" $MODE_FLAGS 'ab:ca:d' ''
assert_outputs '' 'fatal: option -a repeats in options definition for get_options'
...
#shellcheck disable=SC2086
assert_exit_code 2 "$GET_OPTIONS_COMMAND" $MODE_FLAGS ':ab:cd' ''
