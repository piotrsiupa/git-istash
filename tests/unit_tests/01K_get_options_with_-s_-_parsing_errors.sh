...
then
	...
	assert_exit_code 0 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --vidi=qwerty --veni -axdy 
	assert_outputs " -c --vidi 'qwerty' --veni -a -- '-xdy'" ''
	
	__test_section__ 'Unknown long option'
	assert_exit_code 0 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --xyz --vidi=qwerty --veni -ady
	assert_outputs " -c -- '--xyz' '--vidi=qwerty' '--veni' '-ady'" ''
else
	__test_section__ 'Unknown short option'
	assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --vidi=qwerty --veni -axdy 
	assert_outputs '.*' 'error: unknown switch `x'\'
	
	__test_section__ 'Unknown whitespace short option'
	assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --vidi=qwerty --veni -a\ dy 
	assert_outputs '.*' 'error: unknown switch ` '\'
	
	__test_section__ 'Unknown new-line short option'
	assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --vidi=qwerty --veni '-a
dy'
	assert_outputs '.*' 'error: unknown switch `\n'\'
	
	__test_section__ 'Unknown long option'
	assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --xyz --vidi=qwerty --veni -ady
	assert_outputs '.*' 'error: unknown option `xyz'\'
	
	__test_section__ 'Unknown whitespace-only long option'
	assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --"$tab$tab" --vidi=qwerty --veni -ady
	assert_outputs '.*' 'error: unknown option `\t\t'\'
	
	__test_section__ 'Unknown new-line-only long option'
	assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --'
...
assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --vidi=qwerty --veni -ad
...
then
	...
	assert_exit_code 0 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --vidi=qwerty blah --veni -ad
	assert_outputs " -c --vidi 'qwerty' -- 'blah' '--veni' '-ad'" ''
else
	__test_section__ 'Short option without the required argument after a non-option'
	assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --vidi=qwerty blah --veni -ad
	assert_outputs '.*' 'error: switch `d'\'' requires a value'
fi

__test_section__ 'Long option without the required argument'
assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --veni -ady --vidi
...
assert_exit_code 1 run_get_options 'a,ve   ni,b:,vi   di:,c,vi   ci::,d:' -c --ve\ \ \ ni -ady --vi\ \ \ di
...
then
	...
	assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:,   :' -c --veni -ady --\ \ \  #
	assert_outputs '.*' 'error: option `   '\'' requires a value'
fi

__test_section__ 'Abbreviated long option without the required argument'
assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --veni -ady --vid
assert_outputs '.*' 'error: option `vidi'\'' requires a value'

__test_section__ 'Long option with an unexpected argument'
assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --vidi=qwerty --veni=inev -ady
...
assert_exit_code 1 run_get_options "a,ve${tab}ni,b:,vi${tab}di:,c,vi${tab}ci::,d:" -c --vi"$tab"di=qwerty --ve"$tab"ni=inev -ady
assert_outputs '.*' 'error: option `ve\tni'\'' takes no value'

if ! IS_WHITESPACE_STRIPPING_ON
then
	__test_section__ 'Whitespace-only long option with an unexpected argument'
	assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:,   ' -c --vidi=qwerty --\ \ \ =inev -ady
	assert_outputs '.*' 'error: option `   '\'' takes no value'
fi

__test_section__ 'Abbreviated long option with an unexpected argument'
assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' -c --vidi=qwerty --ve=inev -ady
assert_outputs '.*' 'error: option `veni'\'' takes no value'

__test_section__ 'Ambiguous option abbreviation'
assert_exit_code 1 run_get_options 'a,veni,b:,vidi:,c,vici::,d:' --ve -c --vic --vi=qwerty -ady
assert_outputs '.*' 'error: ambiguous option abbreviation `vi'\'

__test_section__ 'Ambiguous option abbreviation with whitespaces'
assert_exit_code 1 run_get_options 'a,ve ni,b:,vi di:,c,vi ci::,d:' --ve -c --vi\ c --vi\ =qwerty -ady
assert_outputs '.*' 'error: ambiguous option abbreviation `vi '\'

if ! IS_WHITESPACE_STRIPPING_ON
then
	__test_section__ 'Ambiguous whitespace-only option abbreviation'
	assert_exit_code 1 run_get_options "a,veni,b:,vidi:,c,vici::,d:,  $tab,   " --ve -c --vic --\ \ =qwerty -ady
	assert_outputs '.*' 'error: ambiguous option abbreviation `  '\'
	
	__test_section__ 'Ambiguous new-line option abbreviation'
	assert_exit_code 1 run_get_options "a,veni,b:,vidi:,c,vici::,d:,$nl$nl ,$nl$nl$tab" --ve -c --vic --"$nl$nl"=qwerty -ady
	assert_outputs '.*' 'error: ambiguous option abbreviation `\n\n'\'
fi

__test_section__ 'Ambiguous option abbreviation when one option is abbreviation of another'
assert_exit_code 1 run_get_options 'a,abcde,b:,xyz,c,abc,d:' --abcd -c --ab -ady
