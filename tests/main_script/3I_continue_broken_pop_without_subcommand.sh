...
git add aaa
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash $QUIET_FLAGS "$CONTINUE_FLAG" $COLOR_FLAGS
...
mv .git/ISTASH_TARGET~ .git/ISTASH_TARGET
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash pop $QUIET_FLAGS "$ABORT_FLAG" $COLOR_FLAGS
