...
PARAMETRIZE_CONTINUE() { # keys
	PARAMETRIZE_OPTION true 'CONTINUE' '' '
			CONTINUE: CONTINUE-SHORT && CONTINUE-LONG && CONTINUE-LONGISH0 & CONTINUE-LONGISH1
		' "$@"
	#shellcheck disable=SC2034
	case "$CONTINUE" in
...
PARAMETRIZE_ABORT() { # keys
	PARAMETRIZE_OPTION true 'ABORT' '' '
			ABORT: && ABORT-LONG && ABORT-LONGISH0 & ABORT-LONGISH1
		' "$@"
	#shellcheck disable=SC2034
	case "$ABORT" in
...
PARAMETRIZE_SUMMARY() { # keys
	PARAMETRIZE_OPTION true 'SUMMARY' 'summary' '
			COMPL: && SUM-DEFAULT & SUM-COMPL-LONG && SUM-COMPL-LONGISH0 & SUM-COMPL-LONGISH2
			| NON-IGNORED: && SUM-NON-IGN-LONG && SUM-NON-IGN-LONGINSH0 & SUM-NON-IGN-LONGINS1
			| NO: && SUM-NO-LONG && SUM-NO-LONGISH0 & SUM-NO-LONGISH1
		' "$@"
	#shellcheck disable=SC2034
	case "$SUMMARY" in
