...
PARAMETRIZE_KEEP_INDEX() { # keys
	PARAMETRIZE_OPTION true 'KEEP_INDEX' '' '
			DEFAULT: && INDEX-DEFAULT &&
			| NO: && INDEX-NO-LONG && INDEX-NO-LONGISH0 & INDEX-NO-LONGISH1
			| YES: INDEX-YES-SHORT && INDEX-YES-LONG && INDEX-YES-LONGISH0 & INDEX-YES-LONGISH1
		' "$@"
	#shellcheck disable=SC2034
	case "$KEEP_INDEX" in
...
PARAMETRIZE_STAGED() { # keys
	PARAMETRIZE_OPTION true 'STAGED' '' '
			YES:
			...
			| NO:
				STAGED-NO-SHORT
				&& STAGED-NO-LONG0 & STAGED-NO-LONG1
				&& STAGED-NO-LONGISH0 & STAGED-NO-LONGISH1 & STAGED-NO-LONGISH2 & STAGED-NO-LONGISH3
					& STAGED-NO-ALT0 & STAGED-NO-ALT1 & STAGED-NO-ALT2 & STAGED-NO-ALT3
					& STAGED-NO-OLD0
			' "$@"
	#shellcheck disable=SC2034
	case "$STAGED" in
...
PARAMETRIZE_UNSTAGED() { # keys
	PARAMETRIZE_OPTION true 'UNSTAGED' '' '
			YES:
			...
			| NO:
				UNSTGD-NO-SHORT
				&& UNSTGD-NO-LONG0 & UNSTGD-NO-LONG1
				&& UNSTGD-NO-LONGISH0 & UNSTGD-NO-LONGISH1 & UNSTGD-NO-LONGISH2 & UNSTGD-NO-LONGISH3
					& UNSTGD-NO-ALT0 & UNSTGD-NO-ALT1 & UNSTGD-NO-ALT2 & UNSTGD-NO-ALT3
					& UNSTGD-NO-OLD0 & UNSTGD-NO-OLD1
			' "$@"
	#shellcheck disable=SC2034
	case "$UNSTAGED" in
...
PARAMETRIZE_ALL() { # keys
	PARAMETRIZE_OPTION true 'ALL' '' '
			DEFAULT: && ALL-DEFAULT &&
			| YES: ALL-YES-SHORT && ALL-YES-LONG &&
		' "$@"
	#shellcheck disable=SC2034
	case "$ALL" in
...
PARAMETRIZE_UNTRACKED() { # keys
	PARAMETRIZE_OPTION true 'UNTRACKED' '' '
			DEFAULT:
			...
			| NO:
				...
				&& UNTR-NO-LONG
				&& UNTR-NO-LONGISH0 & UNTR-NO-LONGISH1
					& UNTR-NO-ALT0 & UNTR-NO-ALT1
			| YES:
				UNTR-YES-SHORT
				&& UNTR-YES-LONG
				&& UNTR-YES-LONGISH0 & UNTR-YES-LONGISH1
					& UNTR-YES-ALT0 & UNTR-YES-ALT1
		' "$@"
	#shellcheck disable=SC2034
	case "$UNTRACKED" in
...
PARAMETRIZE_PATHSPEC_STYLE() { # keys
	...
	PARAMETRIZE_OPTION true 'PATHSPEC' '(full-)?pathspec-style' '
			ARGS:
...
PARAMETRIZE_EXCLUDE() {
	PARAMETRIZE_OPTION true 'EXCLUDE_STYLE' 'partial-options' '
			EXCLUDE:
				
				&& EXCL_LONG
				&& EXCL_STD & EXCL_COLON & EXCL_CARET & EXCL_CARET_COLON
		'
	#shellcheck disable=SC2034
