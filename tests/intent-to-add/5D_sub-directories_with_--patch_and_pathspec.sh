...
then
	#shellcheck disable=SC2086
	new_stash_hash_CO="$(assert_exit_code 0 git istash "$CREATE_OPERATION" $UNTRACKED_FLAGS $ALL_FLAGS $KEEP_INDEX_FLAGS $STAGED_FLAGS $UNSTAGED_FLAGS --patch $EOI ':/a/0' '../a/1/k' '../a/1/l' '0' './1/i' <../.git/answers_for_patch)"
else
	#shellcheck disable=SC2086
