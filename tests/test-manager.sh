...
print_help() {
	...
	printf '\n'
	printf 'Usage: %s command [<options...>] [--] test_name\n' "$(basename "$0")"
	printf '\n'
	...
	printf '\n'
	printf 'Commands:\n'
	printf '\tcreate\t- Add a new and empty test file with given name and rename other\n\t\t  tests to keep the prefixes ordered. '
...
create_test() { # new_test_name
	if [ $# -lt 1 ]
	then
		printf 'You need to specify a test name to create!\n' 1>&2
		return 1
	elif [ $# -gt 1 ]
	then
		printf 'Only 1 argument is allowed!\n' 1>&2
		return 1
	fi
...
delete_test() { # test_name
	if [ $# -lt 1 ]
	then
		printf 'You need to specify a test name to delete!\n' 1>&2
		return 1
	elif [ $# -gt 1 ]
	then
		printf 'Only 1 argument is allowed!\n' 1>&2
