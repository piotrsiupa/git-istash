...
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $COLOR_FLAGS $QUIET_FLAGS $SUMMARY_FLAGS
...
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash "$APPLY_OPERATION" "$CONTINUE_FLAG" $COLOR_FLAGS $QUIET_FLAGS $SUMMARY_FLAGS
