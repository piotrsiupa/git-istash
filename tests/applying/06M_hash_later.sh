...
later_stash_sha="$(git rev-parse stash)"
git stash drop
git reset --hard
...
then
	...
	assert_exit_code 0 git istash apply $QUIET_FLAGS "$later_stash_sha" $SUMMARY_FLAGS $COLOR_FLAGS
	...
	#shellcheck disable=SC2086
	assert_exit_code 1 git istash pop $QUIET_FLAGS "$later_stash_sha" $SUMMARY_FLAGS $COLOR_FLAGS
