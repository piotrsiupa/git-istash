...
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $QUIET_FLAGS $COLOR_FLAGS "$CONTINUE_FLAG" "$ABORT_FLAG" "$QUIT_FLAG"
assert_outputs__apply__continue_abort_quit "$APPLY_OPERATION"
assert_files_HT '
UU aaa		ccc|bbb
   wdf0		wdf0b
!! ignored0	ignored0
