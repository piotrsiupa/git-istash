...
- `06` - Popping/Applying with submodules in the repository.
- `07` - Different ways of specifying which stash to use.
- `08` - Specifying a stash that doesn't exist.
- `09` - Applying commits that are not stashes.
- `10` - Trying to istash when the repository is not in a state that allows that (e.g. an istash is already in progress).
- `11` - Missing / broken data files after conflict resolution.
- `12` - Handling various errors and corner cases.
- `13` - Popping/Applying with sub-directories in the repository and when the working directory is not the repository's root.
