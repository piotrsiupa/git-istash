...
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $QUIET_FLAGS "$CONTINUE_FLAG" $COLOR_FLAGS
...
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash "$OTHER_APPLY_OPERATION" $QUIET_FLAGS "$CONTINUE_FLAG" $SUMMARY_FLAGS $COLOR_FLAGS
