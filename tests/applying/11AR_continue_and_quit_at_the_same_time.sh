...
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash "$APPLY_OPERATION" "$CONTINUE_FLAG" $COLOR_FLAGS "$QUIT_FLAG" $QUIET_FLAGS
assert_outputs__apply__continue_quit "$APPLY_OPERATION"
assert_files_HT '
UU aaa		ccc|bbb
   wdf0		wdf0b
!! ignored0	ignored0
...
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $COLOR_FLAGS "$QUIT_FLAG" $QUIET_FLAGS
