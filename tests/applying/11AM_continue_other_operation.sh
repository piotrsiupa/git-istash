...
assert_exit_code 2 git $ADVICE_FLAGS istash $QUIET_FLAGS "$OTHER_APPLY_OPERATION" $COLOR_FLAGS
...
git add aaa
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash $QUIET_FLAGS "$APPLY_OPERATION" $COLOR_FLAGS "$CONTINUE_FLAG"
...
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash $QUIET_FLAGS $OTHER_APPLY_OPERATION $COLOR_FLAGS "$ABORT_FLAG"
