...
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash $QUIET_FLAGS "$APPLY_OPERATION" "$CONTINUE_FLAG" $COLOR_FLAGS
assert_outputs__apply__data_file_not_1_line "$APPLY_OPERATION" 'ISTASH_WORKING-DIR'
assert_files_HT '
M  aaa		ddd
   wdf0		wdf0b
!! ignored0	ignored0
...
assert_rebase y
assert_dotgit_contents_for "$APPLY_OPERATION" 'ISTASH_WORKING-DIR~'

__test_section__ "Continue $APPLY_OPERATION stash (1)"
mv .git/ISTASH_WORKING-DIR~ .git/ISTASH_WORKING-DIR
stash_sha="$(git rev-parse stash)"
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash $QUIET_FLAGS "$APPLY_OPERATION" $SUMMARY_FLAGS "$CONTINUE_FLAG" $COLOR_FLAGS
