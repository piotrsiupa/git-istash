...
mkdir -p xxx
cd xxx
assert_exit_code 2 capture_outputs "$(get_relative_istash_path)" "$APPLY_OPERATION"
cd -
...
git add aaa xxx/aaa yyy/aaa
cd xxx
assert_exit_code 2 capture_outputs "$(get_relative_istash_path)" "$APPLY_OPERATION" "$CONTINUE_FLAG"
cd -
assert_conflict_message "$APPLY_OPERATION"
assert_files_HT '
UU aaa		eee0|ccc0
