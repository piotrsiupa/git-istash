...
#shellcheck disable=SC2086
assert_exit_code 1 git istash "$APPLY_OPERATION" "$ABORT_FLAG" $COLOR_FLAGS $QUIET_FLAGS
...
mv .git/ISTASH_WORKING-DIR~ .git/ISTASH_WORKING-DIR
#shellcheck disable=SC2086
assert_exit_code 0 git istash "$APPLY_OPERATION" "$ABORT_FLAG" $COLOR_FLAGS $QUIET_FLAGS
