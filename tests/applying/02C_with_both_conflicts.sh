...
assert_exit_code 2 git $ADVICE_FLAGS istash $QUIET_FLAGS "$APPLY_OPERATION" $COLOR_FLAGS $SUMMARY_FLAGS
...
git add aaa
#shellcheck disable=SC2086
assert_exit_code 2 git $ADVICE_FLAGS istash $QUIET_FLAGS "$APPLY_OPERATION" "$CONTINUE_FLAG" $COLOR_FLAGS $SUMMARY_FLAGS
...
#shellcheck disable=SC2086
assert_exit_code 0 git istash $QUIET_FLAGS "$APPLY_OPERATION" "$CONTINUE_FLAG" $COLOR_FLAGS $SUMMARY_FLAGS
