...
printf 'fa4e08a58\n' >.git/ISTASH_TARGET
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $COLOR_FLAGS "$CONTINUE_FLAG" $QUIET_FLAGS
assert_outputs__apply__data_file_invalid_commit "$APPLY_OPERATION" 'ISTASH_TARGET'
assert_files_HT '
M  aaa		ddd
   wdf0		wdf0b
!! ignored0	ignored0
...
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $COLOR_FLAGS "$CONTINUE_FLAG" $SUMMARY_FLAGS $QUIET_FLAGS
