...
printf '5\n' >.git/ISTASH_STASH
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $QUIET_FLAGS $COLOR_FLAGS "$CONTINUE_FLAG"
assert_outputs__apply__data_file_invalid_stash_number "$APPLY_OPERATION" 'ISTASH_STASH' '5'
assert_files_HT '
M  aaa		eee
   wdf0		wdf0b
!! ignored0	ignored0
...
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $QUIET_FLAGS $COLOR_FLAGS "$CONTINUE_FLAG" $SUMMARY_FLAGS
