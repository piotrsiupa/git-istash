...
git add aaa
#shellcheck disable=SC2086
assert_exit_code 1 git istash "$APPLY_OPERATION" "$QUIT_FLAG" $COLOR_FLAGS 0 $QUIET_FLAGS
...
#shellcheck disable=SC2086
assert_exit_code 0 git istash "$APPLY_OPERATION" "$QUIT_FLAG" $COLOR_FLAGS $QUIET_FLAGS
