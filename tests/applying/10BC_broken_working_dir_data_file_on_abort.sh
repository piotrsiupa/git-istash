...
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash "$APPLY_OPERATION" "$ABORT_FLAG" $QUIET_FLAGS $COLOR_FLAGS
...
mv .git/ISTASH_WORKING-DIR~ .git/ISTASH_WORKING-DIR
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash "$APPLY_OPERATION" "$ABORT_FLAG" $QUIET_FLAGS $COLOR_FLAGS
