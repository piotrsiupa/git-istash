...
#shellcheck disable=SC2086
assert_exit_code 1 git istash $QUIET_FLAGS "$APPLY_OPERATION" "$ABORT_FLAG" $COLOR_FLAGS
...
mv .git/ISTASH_WORKING-DIR~ .git/ISTASH_WORKING-DIR
#shellcheck disable=SC2086
assert_exit_code 0 git istash $QUIET_FLAGS $OTHER_APPLY_OPERATION "$ABORT_FLAG" $COLOR_FLAGS
