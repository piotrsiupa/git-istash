...
#shellcheck disable=SC2086
assert_exit_code 1 git $ADVICE_FLAGS istash "$APPLY_OPERATION" "$ABORT_FLAG" $COLOR_FLAGS $QUIET_FLAGS "$QUIT_FLAG"
assert_outputs__apply__abort_quit "$APPLY_OPERATION"
assert_files_HT '
UU aaa		ccc|bbb
   wdf0		wdf0b
!! ignored0	ignored0
...
#shellcheck disable=SC2086
assert_exit_code 0 git $ADVICE_FLAGS istash "$APPLY_OPERATION" $COLOR_FLAGS $QUIET_FLAGS "$ABORT_FLAG"
